...
### SOME CODE PROVIDED BY UCSC CSE30 STAFF

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1j7Aixvsvzy3d1RM7T50gQmjVOficea1H
...
def sudoku_propagate_cell(self, ij):
    ...
    x_cord = (i // 3) * 3
    y_cord = (j // 3) * 3
    for xx in range(x_cord, x_cord + 3):
      for yy in range(y_cord, y_cord + 3):
        if xx == i and yy == j:
          continue
        else:
          newly_singleton.update(self.ruleout(xx, yy, x))
...
# YOUR CODE HERE
to_propagate = ([[[5], [3], [4], [6], [7], [8], [9], [1], [2]], [[6], [7], [2], [1], [9], [5], [3], [4], [8]], [[1], [9], [8], [3], [4], [2], [5], [6], [7]], [[8], [5], [9], [7], [6], [1], [4], [2], [3]], [[4], [2], [6], [8], [5], [3], [7], [9], [1]], [[7], [1], [3], [9], [2], [4], [8], [5], [6]], [[9], [6], [1], [5], [3], [7], [2], [8], [4]], [[2], [8], [7], [4], [1], [9], [6], [3], [5]], [[3], [4], [5], [2], [8], [6], [1], [7], [9]]])
x = (to_propagate.pop())
print(x)
print(x[0] , x[1])
...
def occurs_once_in_sets(set_sequence):
    ...
    final_set = set()
    for coord in set_sequence:
      for item in coord:
        final_dict[item] += 1
    for pairs in final_dict:
      if final_dict[pairs] == 1:
        final_set.add(pairs)
    return final_set
...
def sudoku_where_can_it_go(self):
    ...
    for row in range(9):
      for column in range(9):
        ...
        column_list = []
        cube_list = []
        if len(self.m[row][column]) == 1:
          continue
        for overall_row in range(9):
          row_list.append((self.m[row][overall_row]))
        for overall_column in range(9):
          column_list.append((self.m[overall_column][column]))
        cube_row = (row // 3) * 3
        cube_column = (column // 3) * 3
        for xx in range(cube_row, cube_row + 3):
          for yy in range(cube_column, cube_column + 3):
            cube_list.append((self.m[xx][yy]))
        row_once = occurs_once_in_sets(row_list)
        column_once = occurs_once_in_sets(column_list)
        cube_once = occurs_once_in_sets(cube_list)
        overall_once = row_once | column_once | cube_once
        for value in self.m[row][column]:
          if value in overall_once:
            self.m[row][column] = {value}
            newly_singleton.add((row, column))
            # print(value in occurs_once_in_sets(row_list))
            # print(f"ROW LIST: {row_list}")
            # print((value in occurs_once_in_sets(column_list)))
            # print(f"COLUMN LIST: {column_list}")
            # print(value in occurs_once_in_sets(cube_list))
            # print(row, column)
            # print(value)
